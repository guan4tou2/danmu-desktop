...
| Viewer identity 欄位 | prototype plain input；實作曾做 chip 後回退 plain | 最終互動 pattern（plain 或 chip） |
...
| Desktop (Electron) | Tray Popover | 尚未落地 | 缺頁 |
| Desktop (Electron) | Window Picker | 尚未落地 | 缺頁 |
...
| DS-012 | P2 | 獨立 form-factor | Desktop Tray Popover + Window Picker | 4-6 hr | 各頁交付常態/hover/disabled 狀態 + 尺寸與間距 token |
