...
| 4 | **AdminHistoryPage v2 retrofit** | prototype `admin-batch1.jsx:218` 已設計好「時間軸匯出」(① 時間範圍 chips / ② 內容篩選 toggles / ③ 輸出格式 cards JSON·CSV·SRT / 右側 最近匯出 list) | ✅ **shipped 2026-04-28** — 新模組 `admin-history-v2.js`（349 行）+ 3-tab strip 改成「匯出 v2 / 列表 v1 / 重播」。CSV / JSON / SRT 三格式都 client-side 匯出（重用既有 `/admin/history`）。最近匯出 panel 用 localStorage 暫存 10 筆。日後若要 server-side 共享匯出歷史，補 `/admin/history/exports` endpoint 即可 |

### K.2b RWD / mobile 該不該找 Design 補（user feedback 2026-04-28）

User Q: 「需要叫 design 設計 rwd 嗎 mobile 的樣式」

**現況盤點**

| 介面 | Desktop prototype | Mobile prototype | 工程現況 |
|------|-------------------|------------------|----------|
| Viewer | ✅ `viewer.jsx` | ✅ 同檔內 `mini` variant + `isMobile` prop | ✅ shipped — `<600px` 自動切到 mini |
| Overlay | ✅ `overlay.jsx` / `OverlayIdleQR` | ❌（全螢幕投影/TV，本來就不需要） | ✅ full-screen scales naturally |
| Admin（desktop） | ✅ 30+ pages 全部有 | ❌ **沒設計** RWD breakpoint | 🟡 `style.css` 有 34 個 `@media`，多半是 sidebar collapse / grid wrap，不是「為手機重新排版」 |
| Admin（mobile dedicated） | — | ✅ `admin-batch8.jsx:642 AdminMobilePage`（375×812 phone form） | ❌ **沒做** — §H.1 排 4-6 hr |

**結論：要找 Design 的兩件事**

1. **Admin desktop 的 RWD breakpoint 沒設計**。Prototype 全部是 `1440×900` 桌機版。要不要在窄屏（768px 以下）讓 sidebar collapse / grid 變單欄 / KPI 卡 stack？目前工程是「能擠就擠」式的 fallback，不是「設計過的 mobile admin」。
2. **AdminMobilePage 是獨立的手機 form-factor**（iOS 狀態列 + app header + 訊息 ticker + big action grid），不是 desktop 的 RWD 縮版。要不要 ship？目前 §H.1 排 4-6 hr 但還沒做。

**建議分流**
- 緊急場景（活動主想用手機隨手看訊息流）：先 ship `AdminMobilePage`（4-6 hr 純 FE，prototype 已有），URL 用 `/admin/mobile` 或 UA sniff 自動轉。
- 長期：請 Design 補 admin desktop pages 的 768px / 480px breakpoint artboard。30+ pages 一次補太大，可以挑 3-5 個高頻場景（dashboard / messages / poll / broadcast / history）先做。
