...
### 5.4b 中斷與過期（設計稿 15 · RC1／EX1）

主持人在台上，任何「壞掉」的訊息第一個引發的念頭都是
**「大螢幕是不是也掛了？」**。所以斷線橫幅與登入過期對話框的文案都要
明講「大螢幕與觀眾不受影響」——那不是安慰話，是主持人當下唯一要的資訊。

**登入過期用對話框，不要 `location.reload()`。** reload 會把人踢回一片
空白的登入頁，hash 路由與輸入到一半的東西全沒了。對話框就地重新登入，
成功後不重載——但 `/login` 成功時 server 會 `session.clear()` 再發新的
CSRF token，所以要把 `meta[name=csrf-token]` 換掉（`admin.js` 的
`csrfFetch` 已改成每次重讀 meta，別再把它凍成 const）。

### 5.4c 快速鍵（設計稿 15 · KS1）

**表跟綁定必須是同一份陣列**（`server/static/js/admin-shortcuts.js` 的
`SHORTCUTS`）。列著沒接線的鍵比沒有表更糟——使用者按了沒反應，之後整份表
都不再可信。說明抽屜以前正是這樣，advertise 了 ⌘⇧L／⌘⇧S／⌘⇧C 三個從來
沒有被綁定過的鍵。新增快速鍵就加進那個陣列，不要另外寫 listener。
