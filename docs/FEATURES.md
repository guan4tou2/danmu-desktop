...
- **Desktop 控制 dedicated admin page (`/admin/#/overlay`, legacy `#/broadcast` alias)** — state strip with circular cyan dot + `DESKTOP · ON` + uptime/connections/messages meta + `■ 停止顯示` button (toggles to OFF); session lifecycle (start/end/archive) lives in `#/sessions`. Polestar 2026-05-18: 2-state Desktop display toggle replaces the legacy 4-state broadcast lifecycle.
