// overlay-ws.js — WebSocket client for the overlay (child) window.
//
// Historically this logic lived as an 800-line template string in
// main-modules/child-ws-script.js and was injected via executeJavaScript
// after did-finish-load. It is now a real module in the child bundle
// (overlay-entry.js); the per-session values that used to be string-
// interpolated (IP/port/animation settings/token/idle-screen host+QR)
// arrive as a config object from main via the overlay:get-config invoke.
//
// v5.3.0+: WS merged onto Flask's port. The overlay connects via
// wss://IP:PORT/ws + token query (same port as the web UI).

/** Pure URL builder — kept separate so the wss/token shape is unit-testable. */
function buildWsUrl(ip, port, token) {
  let url = `wss://${ip}:${port}/ws`;
  if (token) {
...
/**
 * Boots the overlay WS client. All connection state lives in this closure —
 * calling it again starts an independent client (main only ever calls it
 * once per overlay window).
 *
 * @param {Object} config - built by main-modules/child-ws-script.js
 *   buildOverlayConfig(): { ip, port, startupAnimationSettings, wsAuthToken,
 *   displayHost, qrSvg }
 */
function initOverlayWs(config) {
  const IP_ADDR = String(config.ip);
  const HTTPS_PORT_NUM = Number(config.port);
  const STARTUP_ANIM_SETTINGS = config.startupAnimationSettings || { enabled: false };
  ...
  function connect() {
    ...
    ws.onclose = (event) => {
      ...
      if (reconnectAttempts < maxReconnectAttempts) {
        ...
        console.log(
          `Attempting to reconnect in ${(delay / 1000).toFixed(1)}s (attempt ${nextAttempt}/${maxReconnectAttempts})...`
        );
    ...
    ws.onmessage = (event) => {
      ...
      if (txt === "connection") {
        ...
        try {
          ...
          if (data.type === "clear") {
            document
              .querySelectorAll("h1.danmu, img.danmu, div.danmu-wrapper, div[style*='translateX']")
          ...
          // Retry loop kept as a safety net: the child bundle defines
          // window.showdanmu synchronously before this module can receive
          // any WS message, but a broken bundle load shouldn't hard-drop
          // danmu without the diagnostic warnings below.
          function processDanmuWhenReady(dataPayload, retries) {
            ...
            if (typeof window.showdanmu === "function") {
              ...
              console.warn(
                "[WebSocket] window.showdanmu not ready, retrying in 100ms... (attempt " + (retries + 1) + "/30)"
              );
  ...
  (function _hydrateIdleScreen() {
    var host = config.displayHost || "";
    var qrSvg = config.qrSvg || "";
...
module.exports = { initOverlayWs, buildWsUrl };
