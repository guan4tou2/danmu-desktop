...
test("admin About server-info values wrap long URLs inside their grid cells", () => {
  ...
  expect(cssSrc).toContain(".admin-about-oss-list { margin-top: var(--space-2); font-family: var(--font-mono); font-size: var(--text-caption); line-height: 1.7; min-width: 0; }");
...
test("admin Backup actions compose shared admin-ui controls", () => {
  ...
  expect(backupSrc).toContain('"/admin/backup/assets/import?dry_run=true"');
  expect(backupSrc).toContain('"/admin/backup/assets/import"');
...
test("admin mono label primitive uses admin-ui naming", () => {
  ...
  expect(jsFiles).not.toContain("admin-v2-monolabel");
