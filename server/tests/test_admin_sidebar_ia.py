...
def test_generated_i18n_bundle_matches_locale_sources(lang):
    """翻譯 bundle 由 `npm run build:i18n` 產生；還帶著退役的 key 就代表有人
    改了 locale JSON 卻沒重新產生——admin 載的是 bundle 不是 JSON，側欄會渲染
    出過期的標籤。

    2026-09-08：翻譯從單一的 `i18n.js` 拆成 `i18n.<lang>.js` 四支（`i18n.js`
    現在只剩 runtime，670 KB → 6 KB，觀眾頁受益最大）。四支都要檢查——只查
    其中一支的話，另外三種語言的側欄過期了也不會有人知道。
    """
    bundle = (
        Path(__file__).resolve().parent.parent / "static" / "js" / f"i18n.{lang}.js"
    ).read_text(encoding="utf-8")

    for key in EXPECTED_GROUP_KEYS:
        assert (
            f'"{key}"' in bundle
        ), f"i18n.{lang}.js missing {key!r} — run `npm run build:i18n` in server/."
    for retired in RETIRED_GROUP_KEYS:
        assert f'"{retired}"' not in bundle, (
            f"i18n.{lang}.js still carries retired key {retired!r} — run "
