...
8. **待辦 · 訊息流工具列與空狀態的文案**——同一區還有幾處違反設計稿 14 的文案
   規則：計數器 `0 TOTAL` / `0.0 MSG/S` 是**寫死在 JS 的英文**（沒進 i18n）、
   篩選列有 `DENSITY` 全大寫、空狀態寫「Desktop 已開啟」而不是詞彙表定案的
   「顯示層」。這批是文案／i18n 工項，不是元件工項。

9. **`@keyframes hud-pulse` 有兩份定義**（style.css 內），後者勝出、前者無法
   觸及。是 hud.css 併入時帶進來的，刪掉無風險但屬於獨立清理。

10. **刻意留著的規格殘留**——`--font-brand: "Unbounded"` 與
...
11. **設計稿 16/17 的收尾**——安全區（OS1）與淺底描邊（OS2）2026-09-07
