...
No domain required. Certificate is auto-generated on first start. / 不需 domain，首次啟動自動產生憑證。
```bash
curl -O https://raw.githubusercontent.com/guan4tou2/danmu-desktop/main/docker-compose.yml
...
```
Exposes `443` (HTTPS) and `80` (redirects to HTTPS). To replace with a real cert, drop `fullchain.pem` / `privkey.pem` into `nginx/certs/` and restart.
